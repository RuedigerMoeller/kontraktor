...
public interface ConcurrentQueueConsumer<E> {
    /**
     * See {@link Queue#poll()} for contract. Elements are removed from the Channel once read.
     * 
     * @return next element or null if Channel is empty
     */
    E poll();
    /**
    ...
    /**
     * This method allows consumers to read from the Channel in batches and thus allows for intelligent handling
     ...
     * 
     * @param bc consumer will be fed elements off the Channel
     * @param batchSizeLimit the maximum number of elements to consume
     */
    void consumeBatch(BatchConsumer<E> bc, int batchSizeLimit);
    
    /**
     * Remove all elements from the Channel. This will not stop the producers from adding new elements, so only
