...
public class DisruptorTest3 {
    ...
    public static class PiJob {
        ...
        public RingBuffer<DRes> ringBuffer;
        ...
        public void calculatePi() {
            ...
            final DRes res = ringBuffer.get(seq);
            res.value = acc;
    ...
    public static class PiResultReclaimer implements EventHandler<DRes> {
        double result;
        public long seq = 0;
        ...
        @Override
        public void onEvent(DRes event, long sequence, boolean isEndOfBatch) throws Exception {
            result += event.value;
            ++seq;
    ...
    public long run(int numTH, int numSlice, int numIter) throws InterruptedException {
        ...
        disruptor.handleEventsWith(procs);

        Disruptor<DRes> resultDisruptor = new Disruptor<>(new EventFactory<DRes>() {
                @Override public DRes newInstance() { return new DRes(); }
                },
                16384,
                executor,
                ProducerType.MULTI,
        ...
        resultDisruptor.handleEventsWith(res);

        disruptor.start();
        resultDisruptor.start();
        ...
        for (int i= 0; i < numSlice; i++ ) {
            ...
            piJob.sliceNr = i;
            piJob.ringBuffer = resultDisruptor.getRingBuffer();
        ...
        disruptor.shutdown();
        resultDisruptor.shutdown();
        executor.shutdownNow();
        return timTest;
    }

    public static void main(String arg[] ) throws InterruptedException {
        final DisruptorTest3 disruptorTest = new DisruptorTest3();
